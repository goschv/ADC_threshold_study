...
# 11reg is the original adjustments as follows:
# swapdim from 0.5 to 1 11reg1swap yielded 0.60; 0.62; 0.32
# trace swap 1; tresh 1 yielded 0.63; 0.66; 0.45
# kernel uses different kernels 3D yielded 0.69; 0.65; 0.36 - boxv 5 yielded 0.71; 0.66; 0.46 - boxv 7 0.63; 0.66; 0.45
# invert adds method to reduce asymmetry by using zero voxels
...
fslmaths $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12_swap_sub_binv.nii.gz -thr 1 $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12_thr2.nii.gz
...
# fill mask --> after dilatation and smoothing it seems unneccessary
...
# apply mask and absolute treshold
...
# In the smoothing process I tried some functions, using a big box or sphere erases shattered voxels better, makes the real infarct appear more defined. Sometimes too restrictive though.
#fslmaths $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged.nii.gz -kernel box 12 -fmean $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12.nii.gz


# Concerning the tresholding for creating a mask I chose the thr 2; it is better than a relative trh. Maybe not optimal yet --> ROC analysis with original ROI?
#fslmaths $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12.nii.gz -thrp 99 $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12_thrp99.nii.gz
#fslmaths $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12.nii.gz -thrP 99 $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12_thrP99.nii.gz


# Double dilatation brings better results but includes more false positive <-- not true results are not always better
#fslmaths $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12_thr2.nii.gz -dilM $s/processed/algo/1_run_pipeline1/TRACEdivADC_merged_kernelbox12_thr2_dilM.nii.gz
